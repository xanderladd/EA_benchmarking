...
orig_params = np.append(orig_params,2)
...
def run_model(stim_ind,real_ind):
    ...
    shutil.copy(volts_fn,'../Data/VHotP'+str(real_ind)+'.dat')
...
def main():
    ...
    param_values = np.zeros_like(param_values)
    cms  = param_values[:,-1]
    param_values = param_values[:,:-1]
    ###### CREATE MAPPING #################
    print(cms)
    allparams_from_mapping(param_values, cms=cms.reshape(-1,1))
    run_model(0,0)
...
#         shutil.move("/tmp/Data/"+ file, "/tmp/" + file)
...
def generate_result_table(nCpus,nodes,pops, sfs, stims, versions, path, title=None, how='vanilla'):
    ...
    for idx,(node,pop,nCpu,stim, sf, vers) in enumerate(zip(nodes,pops,nCpus, stims, sfs, versions)):
        ...
        mean_runtime, std_runtime = np.mean(logRes['runtimes'][1:]), np.std(logRes['runtimes'][1:])
        mean_eval, std_eval = np.mean(logRes['evalTimes'][1:]), np.std(logRes['evalTimes'][1:])
        mean_neuroGPU, std_neuroGPU =  np.mean(logRes['neuroGPUTimes'][1:]), np.std(logRes['neuroGPUTimes'][1:])
        nGpus = logRes['nGpus']

...
#     print(data)
    ...
    time2 = timer.time()
    #curr_target_volt = data[0]
    #curr_data_volt = data
    print("getting stuff took : ", time2-time)
...
def callPara(args):
    ...
    final_res = []
    #for start,stop in zip(range(0, len(args)-15, 15), range(15, len(args), 15)):
#     with Pool(nCpus) as p: # parallel mapping
#         res = p.map(eval_stim_sf_pair,args[0:10])
    with Pool(nCpus) as p: # parallel mapping

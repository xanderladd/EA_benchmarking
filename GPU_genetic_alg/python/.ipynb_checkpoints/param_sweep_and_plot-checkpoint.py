...
#             vc = int(eval(lines[4][:7]))
        pdf = PdfPages('sweep_plots/fig{}.pdf'.format(i))#, vc))
        for idx in range(15):
            fig = plt.figure()
            curr_target_volts =  target_volts_hdf5[i]
            plt.plot(curr_target_volts, color="green", label="target")
            plt.plot(orig_volts, color='black', label="Base")
            for color_idx, color in enumerate(color_rotation):
                ...
                plt.plot(all_volts[start:end], color=color, label=str(transformations[color_idx]), linewidth=.7)
                total_count += 1
            #ax.legend()
                if orig_params[param_idx] == 0:
                    plt.title(labels[idx] + " @ {}".format(full_params[idx]) )
                else:
                    plt.title(labels[idx] + " @ {}".format(orig_params[idx]) )

...
def plotScaling(nCpus,nodes,pops, sfs, stims, versions, how='vanilla', show=True):
    #f  = "runTimeLogs/runTime.log"
    runtimes = []
    labels = []
    for idx,(node,pop,nCpu,stim, sf, vers) in enumerate(zip(nodes,pops,nCpus, stims, sfs, versions)):
...
# def plotStimScaling(nCpus,nodes,stims, versions, show=True):
#     #f  = "runTimeLogs/runTime.log"
#     runtimes = []
#     labels = []
#     inds = np.argsort(stims)
#     versions = versions[inds]
#     nCpus = nCpus[inds]
#     nodes = nodes[inds]
#     stims = stims[inds]
#     for idx,(node,stim,nCpu,vers) in enumerate(zip(nodes,stims,nCpus,versions)):
#         f  = "outputs/{}N_{}C_{}O_{}S_{}SF/{}N_{}C_{}O_{}S_{}SF.log".format(node,nCpu, default_pop, stim, default_sfs, node,nCpu,default_pop, stim, default_sfs)
...
#     plt.xticks(ticks=np.arange(len(runtimes)), labels=labels, rotation=45)
#     plt.ylabel("total runtime (s)")
#     plt.savefig("outputs/stim_scaling.png", bbox_inches='tight')
    
def read_exps(exp_names, condition="vanilla"):
    nodes = []
    pops = []
    nCpus = []
    sfs = []
    ...
    for exp_name in exp_names:
        ...
        if condition == "vanilla" and (curr_stims != default_stims or curr_sfs != default_sfs):
            continue
       # MONKEY PATCH IF STATEMENT, ex; if you have a study with 2N 500 pop
        elif condition == "vanilla" and int(curr_pop) < (int(curr_node)  * POP_SCALING_FACTOR):
            continue
        ...
        log_list = [file for file in os.listdir(agg_exp_path) if ".log" in file and "gpu" not in file]
        if len(log_list) == 0:
            continue
...
def generate_result_table(nCpus,nodes,pops, sfs, stims, versions, how='vanilla'):
    ...
    for idx,(node,pop,nCpu,stim, sf, vers) in enumerate(zip(nodes,pops,nCpus, stims, sfs, versions)):
        ...
        df.loc[idx] = [int(node),int(nCpu)*int(node), 6*int(node), \
                     int(pop),int(stim), int(sf), int(mean_runtime), int(std_runtime), int(pop)/(6*int(node))/float(mean_runtime)]
    df = df.sort_values('nodes', ascending=True)   
    df.to_latex(os.path.join("outputs", figname))
...
def sf_plot_strategy(exp_names, collapse=False):
    if collapse:
        collapse_exps(exp_names)
    nodes, pops,  nCpus, sfs, stims, versions = read_exps(exp_names, condition='sfs')
...
def stim_plot_strategy(exp_names, collapse=False):
    
    if collapse:
        collapse_exps(exp_names)
    nodes, pops,  nCpus, sfs, stims, versions = read_exps(exp_names, condition='stims')
    plt.title("Population Size Scaling w. Nodes")
    # step 1
    plotScaling(nCpus,nodes,pops, sfs, stims, versions, how='stims')
    # step 2
    #wrapProfileMaker(nCpus, nodes, pops, versions)
    
    # step 3
    plot_CPUGPU_bottleneck(nCpus,nodes,pops, sfs, stims, versions, how='stims')

    # step 4
    generate_result_table(nCpus,nodes,pops, sfs, stims, versions, how='stims')
    
def vanilla_plot_strategy(exp_names, collapse=False):
    
    if collapse:
        collapse_exps(exp_names)
    nodes, pops,  nCpus, sfs, stims, versions = read_exps(exp_names)
    plt.title("Population Size Scaling w. Nodes")
    # step 1
    plotScaling(nCpus,nodes,pops, sfs, stims, versions, how='vanilla')
    # step 2
    #wrapProfileMaker(nCpus, nodes, pops, versions)
...
if __name__ == "__main__":
    ...
    if args.stims:
        stim_plot_strategy(exp_names, collapse=True)
    elif args.sfs:
        sf_plot_strategy(exp_names, collapse=True)
    else:
        vanilla_plot_strategy(exp_names, collapse=True)

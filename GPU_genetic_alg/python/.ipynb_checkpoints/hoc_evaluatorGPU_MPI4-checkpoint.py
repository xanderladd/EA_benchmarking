...
#             con.commit()
        ...
        compression = len(fxnsNStims) // size
        

        compressed_args = [args[i:i+compression] for i in range(0,len(args), compression)]
        my_args = compressed_args[global_rank]
        #my_args = my_args[:nCpus] # cut off overflow 
        
        print(len(my_args), "my arg")
        print(len(compressed_args[-1]), "len cp args -1")
        print(len(compressed_args[1]), "len cp args 1")
        start = time.time()
        exs = []
        counter = 0
        res = sf.callPara(my_args)
...
#                 self.data_volts_list = np.reshape(self.data_volts_list, (len(self.opt_stim_list) % nGpus,self.nindv,ntimestep))

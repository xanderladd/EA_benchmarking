...
#         title=" PLOTTING METHOD NOT RECOGNIZED"
    ...
    if os.path.isfile("{}/8{}N_{}C_{}O_{}S_{}SF/G8{}N_{}C_{}O_{}S_{}SF.log".format(path, node,nCpu, pop, stim, sf, node,nCpu,pop, stim, sf)):
        return "{}/8{}N_{}C_{}O_{}S_{}SF/G8{}N_{}C_{}O_{}S_{}SF.log".format(path, node,nCpu, pop, stim, sf, node,nCpu,pop, stim, sf), title
    else:
...
def plotScaling(nCpus,nodes,pops, sfs, stims, versions, path, how='vanilla', title=None, show=True):
    ...
    stds = []
    if title:
        figname = title + "8gpu_scaling.png"
        title = title #+ " Scaling"
    for idx,(node,pop,nCpu,stim, sf, vers) in enumerate(zip(nodes,pops,nCpus, stims, sfs, versions)):
        if int(node) > 8: 
            continue
...
def read_exps(exp_names, condition="vanilla", args=None):
    ...
    for exp_name in exp_names:
        ...
        print(curr_node, curr_core, curr_pop, curr_stims, curr_sfs, version)
...
def generate_result_table(nCpus,nodes,pops, sfs, stims, versions, path, title=None, how='vanilla'):
    ...
    for idx,(node,pop,nCpu,stim, sf, vers) in enumerate(zip(nodes,pops,nCpus, stims, sfs, versions)):
        ...
        if np.isnan(mean_runtime):
            continue
        df.loc[idx] = [int(node),int(nCpu)*int(node), 8*int(node), \
...
def merge_experiments(src, dest, version, path):
    ...
    prof_name = "G8{}N_{}C_{}O_{}S_{}SF.prof".format(curr_node, curr_core, curr_pop, curr_stims, curr_sfs)
    new_prof_name = "G8{}N_{}C_{}O_{}S_{}SF.{}.prof".format(curr_node, curr_core, curr_pop, curr_stims, curr_sfs, version)
    # move profile
    ...
    log_path = os.path.join(dest, "G8{}N_{}C_{}O_{}S_{}SF.log".format(curr_node, curr_core, curr_pop, curr_stims, curr_sfs, version))
    old_log = os.path.join(src_path, "G8{}N_{}C_{}O_{}S_{}SF_{}.log".format(curr_node, curr_core, curr_pop, curr_stims, curr_sfs, version))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path', type=str, required=False, default="8")

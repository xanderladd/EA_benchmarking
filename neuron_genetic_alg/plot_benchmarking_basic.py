    ...
    with open(f, "r") as file:
        ...
        for line in file_lines:
            ...
            if "Generation took" in line:
                if just_started: # skip first gen
                    just_started = False
                    continue
                line = line.split(":")
            ...
            if "neuron" in line and "starts" not in line and "ends" not in line:
                ...
                if "neuronTimes" in locals():
                    neuronTimes = np.append(neuronTimes,  np.array(list(numbers), dtype=np.float32))
                else:
...
#         constraints['N'].remove(curr_node)
    ...
    if len(nodes)< 1:
        print(f' NO EXPERIMENTS FOUND for {args.constraint_file}')
        print(1/0)
...
def generate_result_table(nCpus,nodes,pops, sfs, stims, versions, path, title=None, how='vanilla'):
    df = pd.DataFrame(columns=['Nodes','Total Cpus', 'Total Gpus',\
                               ...
                               'Runtime', 'Runtime Stddev', 'FOM', 'FOM Std Dev','GPU Utilization','Trials'])
    if title:
...
#         import pdb; pdb.set_trace()
        ...
        df.loc[idx] = [int(node),int(nCpu), 0, \
                     int(pop),int(stim), int(sf), float(mean_runtime), float(std_runtime), fom_mean, fom_dev, pct_util, trials]

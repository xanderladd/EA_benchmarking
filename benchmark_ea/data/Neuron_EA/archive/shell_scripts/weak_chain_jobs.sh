...
&& [ "${#offspringArray[@]}" -eq "${#n_sfs[@]}" ]; then
    ...
    for i in "${!nodeArray[@]}"; do
        ...
        # interactive mode
        salloc --nodes $nnodes --qos interactive --time 04:00:00 --constraint haswell --no-shell
        ...
        if [ -z "$JOBID" ] # no dependency
            then
              ...
              jid=$(srun --jobid $id --exclusive=mcs  --signal=USR1@20  --nodes=${nnodes}  --output ${logpath} sh batch_run_cl_cori.sh ${cpu_trial} ${offspring_trial} ${n_stim} ${n_sf} ${nnodes}) 
              JOBID=$(echo $jid | sed 's/[^0-9]*//g')
            else # dependency
             logpath=slurm/${nnodes}N${cpu_trial}C${offspring_trial}O.out
              jid=$(srun --jobid $id --exclusive=mcs  --signal=USR1@20  --nodes=${nnodes} --output ${logpath} sh batch_run_cl_cori.sh ${cpu_trial} ${offspring_trial}  ${n_stim} ${n_sf}  ${nnodes}) # 2 nodes x 8 gpus
              # multiple jobs can depend on a single job
              JOBID=$(echo $jid | sed 's/[^0-9]*//g')
        fi# could be more specific but we should only have one prof here

...
source ./smoke_test_settings


# if smoke test is on, ignore imputs
...
curr_cores=10
echo "run -n $numProcs -c $curr_cores python -m cProfile -o ${nickname}.prof optimize_parameters_genetic_alg.py --offspring_size ${offspring_size} --max_ngen 50  --n_sfs $real_n_sf --n_stims $n_stim  --n_cpus $cores"


# OLD CPROFILE CMD
# srun -n $numProcs -c $curr_cores python -m cProfile -o ${nickname}.prof optimize_parameters_genetic_alg.py --offspring_size ${offspring_size} --max_ngen 50  --n_sfs $real_n_sf --n_stims $n_stim  --n_cpus $cores  & wait

srun -n $numProcs -c $curr_cores python -m benchmark_ea.python.optimize_parameters_genetic_alg --offspring_size ${offspring_size} --max_ngen 1  --n_sfs $real_n_sf --n_stims $n_stim  --n_cpus $cores  --simulator "NeuroGPU"   > ../slurm/${numNodes}N${cores}C${offspring_size}O.out & wait

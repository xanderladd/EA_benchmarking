    ...
    for i in "${!nodeArray[@]}"; do
        ...
        if [ -z "$JOBID" ] # no dependency
            then
              ...
              jid=$(sbatch  --nodes=${nnodes} --ntasks-per-node=8  --output ${logpath} neuron_batch.sh ${cpu_trial} ${offspring_trial} ${n_stim} ${n_sf}) 
              JOBID=$(echo $jid | sed 's/[^0-9]*//g')
            else # dependency
             logpath=slurm/${nnodes}N${cpu_trial}C${offspring_trial}O.out
              jid=$(sbatch --dependency=afterany:$JOBID --nodes=${nnodes} --ntasks-per-node=8  --output ${logpath} neuron_batch.sh ${cpu_trial} ${offspring_trial}  ${n_stim} ${n_sf} ) # 2 nodes x 8 gpus
